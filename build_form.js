...
JSONEditor.defaults.custom_validators.push(function (schema, value, path) {
  ...
  // Extract IGSN string (before " - " if present)
  const extractIGSN = (v) => {
    if (typeof v !== 'string') return '';
    return v.split(' - ')[0].trim();
  };

  const getBuildPrefix = (igsn) => igsn.substring(0, 11);

  const prefixes = new Set();

  value.forEach(v => {
    const igsn = extractIGSN(v);
    if (igsn.length >= 11) {
      prefixes.add(getBuildPrefix(igsn));
    }
  });
  ...
  if (prefixes.size > 1) {
    return [{
      path: path,
      message: "IGSNs should be from the same build (first 11 characters must match)"
    }];
  }
